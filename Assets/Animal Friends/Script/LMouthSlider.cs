...
public class LMouthSlider : MonoBehaviour
{
    public GameObject target;
    public Slider slider;
    public Axis axis = Axis.X;//按照此轴进行移动
    public float left_bottom_back = -0.30f;//左边界
    public float right_up_forward = -0.33f;//右边界
    private float D_value;//左右边界的差值
    public static float left=-0.076f;
    ...
    public void handleValuChange()
    {
        ...
        switch (axis)
        {
            case Axis.X:
                ...
                GameObject LMouth = GameObject.Find("LeftMouthSlider");
                Slider LSlider = LMouth.GetComponent<Slider>();
                GameObject RMouth = GameObject.Find("RightMouthSlider");
                Slider RSlider = RMouth.GetComponent<Slider>();
                RSlider.value = LSlider.value;
                left = target.transform.localPosition.x;
                break;
            case Axis.Y:
